...
Potencjalnie zenonowskie są naszych systemach automaty reprezentujące
interakcję ze środowiskiem zewnętrznym, czyli czujniki. Aby wykluczyć
...
odnieść do rzeczywistości poprzez uznanie, że bardzo bliskie w czasie
...
\subsubsection{Kontroler potoku}
Kontroler potoku odpowiada za sterowanie pracą jednego sygnalizatora.
Komponent jest złożony z jednego automatu, którego struktura jest
ściśle związana z nadawanymi sygnałami. Mamy zatem wierzchołek dla
...
\begin{itemize}
  ...
  \item \texttt{GappedOut} -- cześć rozszerzalna po wykryciu luki (w przypadku, gdy dla
  potoku włączona jest opcja podtrzymania wierzchołek ten jest
  natychmiast opuszczany; w przeciwnym przypadku automat przebywa w
  nim aż do otrzymania polecenia zakończenia od kontrolera fazy)
...
\begin{itemize}
  ...
  od czujnika
\end{itemize}
Podstawowy schemat pracy kontrolera jest następujący:



\subsubsection{Kontroler fazy}
\todo{Składa się z dwóch automatów. Wydzielenie zegara maksymalnego...}
Podstawowy cykl funkcjonowania kontrolera przebiega następująco:
(\imgr{img:models-phase}):
\begin{enumerate}
  \item Kontroler przyjmuje zgłoszenie od jednego z potoków i
  przekazuje informacje o zgłoszeniu przeciwnym kontrolerowi aktywnej
  fazy (lub kontrolerowi cyklu w przypadku spoczynku w czerwonym).
  \item Kontroler dostaje polecenie startu od kontrolera cyklu,
  przekazuje je tym kontrolerom potoków, które zgłosiły
  zapotrzebowanie. Jeśli już w tym momencie jest zapotrzebowanie
  przeciwne, uruchamiany jest zegar czasu maksymalnego.
  \item Kontroler przechodzi w główny stan cyklu, w którym obsługuje
  następujące komunikaty od kontrolerów potoków:
  \begin{itemize}
    \item informacje o wykryciu luki,
    \item informacje o zakończeniu pracy,
    \item zgłoszenia późne.
  \end{itemize}
  ...
  \item Aktywność fazy może zostać zakończona z dwóch powodów:\todo{Dopisać.}
  \begin{itemize}
    \item W momencie, gdy każdy z kontrolerów potoku przekazał
    informację bądź o wykryciu luki bądź o zakończeniu pracy,
    kontroler fazy przechodzi w ostatni etap swojej pracy. Wysyła
    wszystkim podtrzymanym potokom polecenie zakończenia pracy i po
    otrzymaniu potwierdzenia jej zakończenia sam przechodzi w jeden ze
    stanów nieaktywności, informując o tym kontroler cyklu.
  \end{itemize}
\end{enumerate}
Podobnie jak na poziomie opisu, działanie kontrolera fazy
jednopotokowej jest znacznie prostsze. Zapewnia on jedynie poprawną
pracę zegara czasu maksymalnego, nie pełni natomiast funkcji
związanych z koordynacją wielu potoków (tj. zgłoszeń późnych i podtrzymania).

\todo{Należy podkreślić fakt, że kontrolery fazy są komponentami
  niezależnymi czasowo.}

\subsubsection{Kontroler cyklu}
Funkcjonowanie kontrolera cyklu..
Kontroler uruchamia fazę na którą jest zapotrzebowanie, następnie zaś
czeka na potwierdzenie zakończenia jej aktywności.
